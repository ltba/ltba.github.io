...
$(function(){
        ...
        layer.prompt(
        {
            ...
            btn3: function(index, layero){
                layer.open({
                    ...
                    '5.导入本地歌单(可无视)<input type="file" id="file" /><input type="button" value="开始导入" onclick="readAsText();"/>'
                });  
            }
        },
    ...
    // 图片加载失败处理
    $('img').error(function(){
        $(this).attr('src', 's/player_cover.png');
    });
...
function addSheet(no, name, cover) {
    if(!cover) cover = "s/player_cover.png";
    if(!name) name = "读取中...";
...
function clearDislist() {
    ...
    if(rem.dislist == 1) {  // 正在播放列表
        ...
        $(".sheet-item[data-no='1'] .sheet-cover").attr('src', 's/player_cover.png');    // 恢复正在播放的封面
